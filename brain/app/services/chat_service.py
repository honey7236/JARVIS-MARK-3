...
class ChatService:
    ...
    def load_session_from_disk(self, session_id: str) -> bool:
        ...
        """
        # Sanitize ID for use in filenames (no dashes or spaces).
    ...
    def process_message(self, session_id: str, user_message: str) -> str:
        """
        Handle one general-chat message:
        - Add user message
        - Call Groq (no web search)
        - Add assistant reply
        - Return response
        """
        ...
        # Format chat history for LLM (exclude latest user message)
        chat_history = self.format_history_for_llm(
            session_id,
            exclude_last=True
        )

        # Get response from Groq (general mode)
    ...
    def process_realtime_message(self, session_id: str, user_message: str) -> str:
        """
        Handle one realtime message:
        - Add user message
        - Call realtime service (Tavily + Groq)
        - Add assistant reply
        - Return response
        ...
        # Get realtime response (Tavily search + Groq)
    ...
    def save_chat_session(self, session_id: str):
        ...
        Called after each message so conversation is persisted.
        The vector store is rebuilt on startup from these files.
        If session is missing or empty, do nothing.
        On write error, only log.
        """
